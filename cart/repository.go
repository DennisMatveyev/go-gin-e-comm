...
func (r *cartRepository) GetCart(c context.Context, userID primitive.ObjectID) (*Cart, error) {
	...
	if err != nil {
		...
		slog.Error("Failed to get cart", slog.String("error", err.Error()))
...
func (r *cartRepository) AddProductToCart(c context.Context, userID, productID primitive.ObjectID, quantity int) error {
	...
	if err != nil {
		slog.Error("Failed to add product to cart", slog.String("error", err.Error()))
...
func (r *cartRepository) RemoveProductFromCart(c context.Context, userID, productID primitive.ObjectID) error {
	...
	if len(cart.Items) == 0 {
		...
		if err != nil {
			slog.Error("Failed to remove cart", slog.String("error", err.Error()))
			return common.ErrDatabase
		}
		return nil
	}
	...
	if err != nil {
		slog.Error("Failed to remove product from cart", slog.String("error", err.Error()))
		return common.ErrDatabase
	}
	return nil
}
...
func (r *cartRepository) Checkout(c context.Context, userID primitive.ObjectID) (*Order, error) {
	...
	for _, item := range cart.Items {
		...
		if err != nil {
			if err == mongo.ErrNoDocuments {
				slog.Error("Product not found", slog.String("product_id", item.ProductID.Hex()))
	...
	_, err = r.db.Collection("carts").DeleteOne(c, bson.M{"user_id": userID})
	if err != nil {
		slog.Error("Failed to remove cart after checkout", slog.String("error", err.Error()))
		return nil, common.ErrDatabase
	}
	return order, nil
}
